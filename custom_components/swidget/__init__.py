...
async def async_discover_devices(hass: HomeAssistant) -> dict[str, SwidgetDevice]:
    ...
    _LOGGER.info(f"Swidget Discovery found {len(discovered_devices)} device(s)")
...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    try:
        _LOGGER.debug(f"Setup Data: {entry.data}")
...
async def async_unload_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    _LOGGER.debug(f"Unloading entry for device: {device}")
